...
class GoogLeNetAE(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(1024, self.out_channels, kernel_size=4),              # [batch, out, 16, 16]  Added
            nn.Conv2d(self.out_channels, self.out_channels, kernel_size=3), # [batch, out, 16, 16]  Added
        )
...
class ResNetAE(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, block=BasicBlock, layers=[2,2,2,2], zero_init_residual=False,
        ...
        self.encoder = nn.Sequential(
            ...
            #self._make_layer(block, 512, layers[3], stride=1, dilate=replace_stride_with_dilation[2]),
            nn.AdaptiveAvgPool2d((16, 16)),                                                  # [batch, 512, 16, 16]
            nn.Conv2d(256, self.out_channels, kernel_size=3, padding=1)                      # [batch, out, 16, 16]   Added
        )
        ''' -Changed: ResNet 18 in use
            -making layers with stride 1
            -adaptative pooling gets desired feature size 
            -took away last set of layers to make the net faster
            -adding 1 Conv2d layer to get output channels right '''
    ...
    def forward(self, x):
        encoded = self.encoder(x.view(x.shape[0], self.in_channels, 96, 96))
        return encoded.view(x.shape[0], self.out_channels, 16, 16)
...
class SplitBrain(nn.Module):
    def __init__(self, encoder="alex", num_ab=25, num_L=100):
        super(SplitBrain, self).__init__()
        if encoder == "alex":
            self.ab_net = AlexNetAE(in_channels=2, out_channels=num_L)
            self.L_net = AlexNetAE(in_channels=1, out_channels=num_ab**2)
        elif encoder == "resnet":
            self.ab_net = ResNetAE(in_channels=2, out_channels=num_L)
            self.L_net = ResNetAE(in_channels=1, out_channels=num_ab**2)
        elif encoder == "googl":
            self.ab_net = GoogLeNetAE(in_channels=2, out_channels=num_L)
            self.L_net = GoogLeNetAE(in_channels=1, out_channels=num_ab**2)
        elif encoder == "simple":
            self.ab_net = SimpleAE(in_channels=2, out_channels=num_L)
            self.L_net = SimpleAE(in_channels=1, out_channels=num_ab**2)
        print("Split Brain Parameters- AB Net: ", sum(p.numel() for p in self.ab_net.parameters() if p.requires_grad))
        print("Split Brain Parameters- L Net: ", sum(p.numel() for p in self.L_net.parameters() if p.requires_grad))

    def forward(self, x):
        ab, L = x
        ab_hat = self.L_net(L)
        L_hat = self.ab_net(ab)
        return ab_hat, L_hat

''' Train a classifier consisting of the AlexNet plus a resizing linear layer'''
class SBNetClassifier(nn.Module):
    def __init__(self, encoder="alex", classifier="mlp", num_ab=25, num_L=100):
        super(SBNetClassifier, self).__init__()
        self.sp = SplitBrain(encoder=encoder, num_ab=num_ab, num_L=num_L)
        n_in = num_ab**2+num_L
        if encoder == "alex":
            n_in *= 11**2
        elif encoder == "googl":
    ...
    def forward(self, x):
        ab, L = x
        encoded_L = self.sp.ab_net(ab.view(ab.shape[0], self.sp.ab_net.in_channels, 96, 96))
        encoded_ab = self.sp.L_net(L.view(L.shape[0], self.sp.L_net.in_channels, 96, 96))
        full_emb = torch.cat((encoded_ab, encoded_L), 1)
...
self.fc = nn.Sequential(
            nn.Linear(self.out_channels*12*12, 4096),
            nn.BatchNorm1d(4096),
            nn.ReLU(inplace=True),
            nn.Linear(4096,4096)
        )
        ...
        self.upsample = nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=4, stride=4, padding=1)
        )
'''